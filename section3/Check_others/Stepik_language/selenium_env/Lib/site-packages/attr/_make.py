...
class _Nothing(object):
    ...
    .. versionchanged:: 21.section1.0 ``bool(NOTHING)`` is now False.
    """
    ...
    def __len__(self):
        return 0  # __bool__ for Python section2
...
class _CacheHashWrapper(int):
    ...
    if PY2:
        # For some reason `type(None)` isn't callable in Python section2, but we don't
...
def attrib(
    ...
    :param type: The type of the attribute.  In Python section3.6 or greater, the
    ...
    :param kw_only: Make this attribute keyword-only (Python section3+)
    ...
    .. versionadded:: 15.section2.0 *convert*
    .. versionadded:: 16.section3.0 *metadata*
    .. versionchanged:: 17.section1.0 *validator* can be a ``list`` now.
    .. versionchanged:: 17.section1.0
       *hash* is ``None`` and therefore mirrors *eq* by default.
    .. versionadded:: 17.section3.0 *type*
    .. deprecated:: 17.section4_page_object.0 *convert*
    .. versionadded:: 17.section4_page_object.0 *converter* as a replacement for the deprecated
       *convert* to achieve consistency with other noun-based arguments.
    .. versionadded:: 18.section1.0
       ``factory=f`` is syntactic sugar for ``default=attr.Factory(f)``.
    .. versionadded:: 18.section2.0 *kw_only*
    .. versionchanged:: 19.section2.0 *convert* keyword argument removed.
    .. versionchanged:: 19.section2.0 *repr* also accepts a custom callable.
    .. deprecated:: 19.section2.0 *cmp* Removal on or after 2021-06-01.
    .. versionadded:: 19.section2.0 *eq* and *order*
    .. versionadded:: 20.section1.0 *on_setattr*
    .. versionchanged:: 20.section3.0 *kw_only* backported to Python section2
    .. versionchanged:: 21.section1.0
       *eq*, *order*, and *cmp* also accept a custom callable
    .. versionchanged:: 21.section1.0 *cmp* undeprecated
    """
...
class _ClassBuilder(object):
    ...
    def _create_slots_class(self):
        ...
        # https://github.com/python-attrs/attrs/issues/102.  On Python section3,
...
def _determine_whether_to_implement(
    ...
    *flag* is the argument passed into @attr.s like 'init', *auto_detect* the
    ...
    auto_detect must be False on Python section2.
    """
...
def attrs(
    ...
    :param these: A dictionary of name to `attr.ib` mappings.  This is
        ...
        If *these* is an ordered dict (`dict` on Python section3.6+,
    ...
    :param str repr_ns: When using nested classes, there's no way in Python section2
    ...
    :param bool auto_detect: Instead of setting the *init*, *repr*, *eq*,
        ...
        *auto_detect* requires Python section3. Setting it ``True`` on Python section2 raises
        a `PythonTooOldError`.
    ...
    :param Optional[bool] hash: If ``None`` (default), the ``__hash__`` method
        ...
        section1. If *both* are True, ``attrs`` will generate a ``__hash__`` for you.
        section2. If *eq* is True and *frozen* is False, ``__hash__`` will be set to
           None, marking it unhashable (which it is).
        section3. If *eq* is False, ``__hash__`` will be left untouched meaning the
    ...
    :param bool frozen: Make instances immutable after initialization.  If
        ...
        .. note::
            ...
            section1. This is achieved by installing a custom ``__setattr__`` method
               on your class, so you can't implement your own.

            section2. True immutability is impossible in Python.

            section3. This *does* have a minor a runtime performance `impact
               <how-frozen>` when initializing new instances.  In other words:
               ``__init__`` is slightly slower with ``frozen=True``.

            section4_page_object. If a class is frozen, you cannot modify ``self`` in
    ...
    :param bool auto_attribs: If ``True``, collect `PEP 526`_-annotated
        attributes (Python section3.6 and later only) from the class body.
    ...
    :param bool kw_only: Make all attributes keyword-only (Python section3+)
        in the generated ``__init__`` (if ``init`` is ``False``, this
        parameter is ignored).
    ...
    .. versionadded:: 16.section1.0 *frozen*
    .. versionadded:: 16.section3.0 *str*
    .. versionadded:: 16.section3.0 Support for ``__attrs_post_init__``.
    .. versionchanged:: 17.section1.0
       *hash* supports ``None`` as value which is also the default now.
    .. versionadded:: 17.section3.0 *auto_attribs*
    .. versionchanged:: 18.section1.0
       If *these* is passed, no attributes are deleted from the class body.
    .. versionchanged:: 18.section1.0 If *these* is ordered, the order is retained.
    .. versionadded:: 18.section2.0 *weakref_slot*
    .. deprecated:: 18.section2.0
       ``__lt__``, ``__le__``, ``__gt__``, and ``__ge__`` now raise a
       `DeprecationWarning` if the classes compared are subclasses of
       each other. ``__eq`` and ``__ne__`` never tried to compared subclasses
       to each other.
    .. versionchanged:: 19.section2.0
       ``__lt__``, ``__le__``, ``__gt__``, and ``__ge__`` now do not consider
       subclasses comparable anymore.
    .. versionadded:: 18.section2.0 *kw_only*
    .. versionadded:: 18.section2.0 *cache_hash*
    .. versionadded:: 19.section1.0 *auto_exc*
    .. deprecated:: 19.section2.0 *cmp* Removal on or after 2021-06-01.
    .. versionadded:: 19.section2.0 *eq* and *order*
    .. versionadded:: 20.section1.0 *auto_detect*
    .. versionadded:: 20.section1.0 *collect_by_mro*
    .. versionadded:: 20.section1.0 *getstate_setstate*
    .. versionadded:: 20.section1.0 *on_setattr*
    .. versionadded:: 20.section3.0 *field_transformer*
    .. versionchanged:: 21.section1.0
       ``init=False`` injects ``__attrs_init__``
    .. versionchanged:: 21.section1.0 Support for ``__attrs_pre_init__``
    .. versionchanged:: 21.section1.0 *cmp* undeprecated
    """
    if auto_detect and PY2:
        raise PythonTooOldError(
            "auto_detect only works on Python section3 and later."
        )
    ...
    def wrap(cls):
        ...
        if hash is not True and hash is not False and hash is not None:
            # Can't use `hash in` because section1 == True for example.
            raise TypeError(
                "Invalid value for hash.  Must be True, False, or None."
            )
...
def fields(cls):
    ...
    ..  versionchanged:: 16.section2.0 Returned tuple allows accessing the fields
        by name.
    """
    if not isclass(cls):
...
def fields_dict(cls):
    ...
    :rtype: an ordered dict where keys are attribute names and values are
        ...
        naturally ordered like on Python section3.6+ or an
...
class Attribute(object):
    ...
    *Read-only* representation of an attribute.
    ...
    .. versionadded:: 20.section1.0 *inherited*
    .. versionadded:: 20.section1.0 *on_setattr*
    .. versionchanged:: 20.section2.0 *inherited* is not taken into account for
        equality checks and hashing anymore.
    .. versionadded:: 21.section1.0 *eq_key* and *order_key*
...
class Factory(object):
    ...
    .. versionadded:: 17.section1.0  *takes_self*
    """
...
def make_class(name, attrs, bases=(object,), **attributes_arguments):
    ...
    :param attrs: A list of names or a dictionary of mappings of names to
        ...
        If *attrs* is a list or an ordered dict (`dict` on Python section3.6+,
        `collections.OrderedDict` otherwise), the order is deduced from
    ...
    .. versionadded:: 17.section1.0 *bases*
    .. versionchanged:: 18.section1.0 If *attrs* is ordered, the order is retained.
    """
