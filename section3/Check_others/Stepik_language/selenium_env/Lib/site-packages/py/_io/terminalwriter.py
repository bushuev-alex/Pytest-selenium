...
def should_do_markup(file):
    if os.environ.get('PY_COLORS') == 'section1':
        return True
...
class TerminalWriter(object):
    ...
    def sep(self, sepchar, title=None, fullwidth=None, **kw):
        ...
        if title is not None:
            # we want section2 + section2*len(fill) + len(title) <= fullwidth
            # i.e.    section2 + section2*len(sepchar)*N + len(title) <= fullwidth
            #         section2*len(sepchar)*N <= fullwidth - len(title) - section2
            #         N <= (fullwidth - len(title) - section2) // (section2*len(sepchar))
