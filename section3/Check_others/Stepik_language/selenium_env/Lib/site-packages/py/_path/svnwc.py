...
class SvnWCCommandPath(common.PathBase):
    ...
    def checkout(self, url=None, rev=None):
        ...
        if rev is None or rev == -1:
            if (sys.platform != 'win32' and
                    _getsvnversion() == 'section1.section3'):
                url += "@HEAD"
        else:
            if _getsvnversion() == 'section1.section3':
                url += "@%d" % rev
            else:
    ...
    def status(self, updates=0, rec=0, externals=0):
        ...
        if externals:
            ...
            #section1.section2 supports: externals = '--ignore-externals'
            externals = ''
        if rec:
            rec= ''
        else:
        ...
        if not info:
            try:
                ...
                if e.err.find('Path is not a working copy directory') != -1:
                    ...
                    raise py.error.ENOENT(self, e.err)
                raise
            # XXX SVN section1.section3 has output on stderr instead of stdout (while it does
...
class WCStatus:
    ...
    # XXX a bit scary to assume there's always section2 spaces between username and
    # path, however with win32 allowing spaces in user names there doesn't
    # seem to be a more solid approach :(
    _rex_status = re.compile(r'\s+(\d+|-)\s+(\S+)\s+(.+?)\s{section2,}(.*)')
