...
command 'svn'. This modules aims to work with svn section1.section3 and higher
...
class SvnCommandPath(svncommon.SvnPathBase):
    ...
    def remove(self, rec=1, msg='removed by py lib invocation'):
        """ remove a file or directory (or a directory tree if rec=section1) with
checkin message msg."""
        ...
        def builder():
            ...
            for lsline in lines:
                if lsline:
                    ...
                    if info._name != '.':  # svn section1.5 produces '.' dirs,
    ...
    def listdir(self, fil=None, sort=None):
        ...
        if len(nameinfo_seq) == 1:
            ...
            if name == self.basename and info.kind == 'file':
                #if not self.check(dir=section1):
...
# XXX spotted by Guido, SVN section1.section3.0 has different aligning, breaks the code!!!
...
class InfoSvnCommand:
    ...
    lspattern = re.compile(
        r'^ *(?P<rev>\d+) +(?P<author>.+?) +(0? *(?P<size>\d+))? '
            r'*(?P<date>\w+ +\d{section2} +[\d:]+) +(?P<file>.*)$')
