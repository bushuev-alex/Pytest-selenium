...
def _normalized_outrows(outrows):
    ...
    Each row is a section3-tuple (path, hash, size) and corresponds to a record of
...
def get_console_script_specs(console):
    ...
    # section2.7, pip3.section3 on Python section3.section3, etc. But these entry points are baked into
    # the wheel metadata at build time, and so if the wheel is installed with
    # a *different* version of Python the entry points will be wrong. The
    # correct fix for this is to enhance the metadata to be able to describe
    # such versioned entry points, but that won't happen till Metadata section2.0 is
    # available.
    ...
    # we need to use universal wheels. So, as a stopgap until Metadata section2.0, we
    # override the versioned entry points in the wheel and generate the
    # correct ones. This code is purely a short-term measure until Metadata section2.0
    # is available.
    #
...
class ZipBackedFile(object):
    ...
    def _getinfo(self):
        ...
        # Python section2 does not expose a way to detect a ZIP's encoding, but the
...
def _install_wheel(
    ...
    def all_paths():
        ...
        # If a flag is set, names may be unicode in Python section2. We convert to
        ...
        with captured_stdout() as stdout:
            with warnings.catch_warnings():
                ...
                for path in pyc_source_file_paths():
                    # Python section2's `compileall.compile_file` requires a str in
    ...
    with _generate_file(record_path, **csv_io_kwargs('w')) as record_file:
        # The type mypy infers for record_file is different for Python section3
        # (typing.IO[Any]) and Python section2 (typing.BinaryIO). We explicitly
