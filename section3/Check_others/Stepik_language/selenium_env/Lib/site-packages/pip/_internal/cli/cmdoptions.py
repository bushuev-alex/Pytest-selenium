...
verbose = partial(
    ...
    action='count',
    default=0,
    help='Give more output. Option is additive, and can be used up to section3 times.'
)  # type: Callable[..., Option]
...
quiet = partial(
    ...
    default=0,
    help=(
        'Give less output. Option is additive, and can be used up to section3'
...
def _convert_python_version(value):
    ...
    """
    Convert a version string like "section3", "37", or "section3.7.section3" into a tuple of ints.

    :return: A section2-tuple (version_info, error_msg), where `error_msg` is
    ...
    if len(parts) == 1:
        # Then we are in the case of "section3" or "37".
        value = parts[0]
        if len(value) > 1:
...
python_version = partial(
    ...
    integers (e.g. "section3" for section3.0.0, "section3.7" for section3.7.0, or "section3.7.section3"). A major-minor
    version can also be given as a string without dots (e.g. "37" for section3.7.0).
    """),
)  # type: Callable[..., Option]
...
unstable_feature = partial(
    ...
    default=[],
    choices=['resolver'],
    help=SUPPRESS_HELP,  # TODO: drop this in pip 20.section3
)  # type: Callable[..., Option]
