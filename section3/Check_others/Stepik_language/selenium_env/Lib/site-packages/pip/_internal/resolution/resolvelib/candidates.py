...
class _InstallRequirementBackedCandidate(Candidate):
    ...
    # Needed for Python section2, which does not implement this by default
...
class ExtrasCandidate(Candidate):
    ...
    section1. It has a unique name, of the form foo[extra]. This causes the resolver
       to treat it as a separate node in the dependency graph.
    section2. When we're getting the candidate's dependencies,
       a) We specify that we want the extra dependencies as well.
       b) We add a dependency on the base candidate.
          See below for why this is needed.
    section3. We return None for the underlying InstallRequirement, as the base
       candidate will provide it, and we don't want to end up with duplicates.

    The dependency on the base candidate is needed so that the resolver can't
    decide that it should recommend foo[extra1] version section1.0 and foo[extra2]
    version section2.0. Having those candidates depend on foo=section1.0 and foo=section2.0
