...
# Fallback to noop_lru_cache in Python section2
# TODO: this can be removed when python section2 support is dropped!
...
def _get_html_response(url, session):
    ...
    section1. If the URL looks suspiciously like an archive, send a HEAD first to
       check the Content-Type is HTML, to avoid downloading a large file.
       Raise `_NotHTTP` if the content type cannot be determined, or
       `_NotHTML` if it is not HTML.
    section2. Actually perform the request. Raise HTTP exceptions on network failures.
    section3. Check the Content-Type header to make sure we got HTML, and raise
       `_NotHTML` otherwise.
    """
...
def _create_link_from_element(
    ...
    if yanked_reason:
        # This is a unicode string in Python section2 (and section3).
...
def _get_html_page(link, session=None):
    ...
    if session is None:
        raise TypeError(
            "_get_html_page() missing section1 required keyword argument: 'session'"
        )
...
class CollectedLinks(object):
    ...
    (section1) links from the configured file locations,
    (section2) links from the configured find_links, and
    (section3) urls to HTML project pages, as described by the PEP 503 simple
        repository API.
    """
