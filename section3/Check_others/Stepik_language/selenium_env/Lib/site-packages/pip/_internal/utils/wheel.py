...
def pkg_resources_distribution_for_wheel(wheel_zip, name, location):
    ...
    for path in metadata_files:
        # If a flag is set, namelist entries may be unicode in Python section2.
...
def wheel_dist_info_dir(source, name):
    ...
    Raises AssertionError or UnsupportedWheel if not found, >section1 found, or
...
def check_compatibility(version, name):
    ...
    ahead of what it's compatible with (e.g section2.0 > section1.section1); and warn when
    installing a version only minor version ahead (e.g section1.section2 > section1.section1).

    version: a section2-tuple representing a Wheel-Version (Major, Minor)
