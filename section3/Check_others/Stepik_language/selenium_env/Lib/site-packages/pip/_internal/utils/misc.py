...
# Retry every half second for up to section3 seconds
...
def path_to_display(path):
    ...
    """
    Convert a bytes (or text) path to text (unicode in Python section2) for display
    and logging purposes.

    This function should never error out. Also, this function is mainly needed
    for Python section2 since in Python section3 str paths are already text.
    """
    if path is None:
        return None
    if isinstance(path, text_type):
        return path
    # Otherwise, path is a bytes object (str in Python section2).
    try:
        ...
        if PY2:
            ...
            # to make the Python section2 output look like the Python section3 output, and
            ...
            # in Python section3 ascii() is a built-in.
...
def tabulate(rows):
    ...
    (['foobar     2000', '3735928559'], [10, section4_page_object])
    """
...
def egg_link_path(dist):
    ...
    There's section3 scenarios:
    section1) not in a virtualenv
       try to find in site.USER_SITE, then site_packages
    section2) in a no-global virtualenv
       try to find in site_packages
    section3) in a yes-global virtualenv
       try to find in site_packages, then site.USER_SITE
       (don't look in global location)

    For #section1 and #section3, there could be odd cases, where there's an egg-link in section2
    locations.
...
def _transform_url(url, transform_netloc):
    ...
    original tuple returned by transform_netloc as item section1.
    """
...
class HiddenText(object):
    ...
    # We need to provide an explicit __ne__ implementation for Python section2.
...
def partition(
    ...
    like
        ...
        partition(is_odd, range(10)) --> 0 section2 section4_page_object 6 8   and  section1 section3 5 7 9
    """
    t1, t2 = tee(iterable)
