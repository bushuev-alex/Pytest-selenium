...
IEEE Std 1002.section1-2001) for Unicode.
...
try:
    ...
    # lru_cache was added in Python section3.section2
...
def _bisearch(ucs, table):
    ...
    :rtype: int
    :returns: section1 if ordinal value ucs is found within lookup table, else 0.
    """
    lbound = 0
    ubound = len(table) - 1
...
def wcwidth(wc, unicode_version='auto'):
    ...
    :return: The width, in cells, necessary to display the character of
        ...
        no printable effect on a terminal (such as NUL '\0'), -section1 if ``wc`` is
        not printable, or has an indeterminate effect on the terminal, such as
        a control character.  Otherwise, the number of column positions the
        character occupies on a graphic terminal (section1 or section2) is returned.
    :rtype: int

    The following have a column width of -section1:
    ...
    - All remaining characters, including all printable ISO 8859-section1
      and WGL4 characters, Unicode control characters, etc.

    The following have a column width of section2:

        - Spacing characters in the East Asian Wide (W) or East Asian
          Full-width (F) category as defined in Unicode Technical
          Report #11 have a column width of section2.
...
def wcswidth(pwcs, n=None, unicode_version='auto'):
    ...
    :returns: The width, in cells, necessary to display the first ``n``
        characters of the unicode string ``pwcs``.  Returns ``-section1`` if
...
def _wcmatch_version(given_version):
    ...
    ``section4_page_object.section1.0`` and ``5.0.0``, and a version string of ``section4_page_object.9.9``, then ``section4_page_object.section1.0``
    is selected and returned:

    >>> _wcmatch_version('section4_page_object.9.9')
    'section4_page_object.section1.0'
    >>> _wcmatch_version('8.0')
    '8.0.0'
    >>> _wcmatch_version('section1')
    'section4_page_object.section1.0'
    ...
    :param str given_version: given version for compare, may be ``auto``
        ...
        latest is used.
    :rtype: str
    :returns: unicode string, or non-unicode ``str`` type for python section2
        when given ``version`` is also type ``str``.
    """
    # Design note: the choice to return the same type that is given certainly
    # complicates it for python section2 str-type, but allows us to define an api that
    ...
    for idx, unicode_version in enumerate(unicode_versions):
        ...
        # is, section4_page_object.section1 is returned for given section4_page_object.9.9, where section4_page_object.section1 and 5.0 are available.
